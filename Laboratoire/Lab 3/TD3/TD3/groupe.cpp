...
Groupe& Groupe::ajouterDepense(Depense* depense, Utilisateur* payePar, vector<Utilisateur*> payePour)
{
	...
	if (!aUneErreur) {
		...
		*payePar += depenseGroupe;
		comptes_[index[index.size() - 1]] += (depenseGroupe->getMontant() - depenseGroupe->getMontantPersonnel());
		payePar->calculerTotalDepenses();
		//Recalculer le taux si l'utilisateur est premium
		if (payePar->getType() == Premium)
			static_cast<UtilisateurPremium*>(payePar)->calculerTaux();

		for (unsigned i = 0; i < payePour.size(); i++)
		{
			*payePour[i] += depenseGroupe;
	...
	{
		cout << "Erreur : Vous tenter d'ajouter une depense individuelle au groupe ou alors les personnes impliquees dans la depense de groupe ne sont pas dans le groupe." << endl << endl;
	}
	return *this;
}
...
{
	//Ajouter le compte de l'utilisateur
	comptes_.push_back(0);

	//Si l'utilisateur est premium, vérifier s'il lui reste des jours
	if (utilisateur->getType() == Premium) {
		if (static_cast<UtilisateurPremium*>(utilisateur)->getJoursRestants() != 0)
			utilisateurs_.push_back(utilisateur);
		else
			cout << "Erreur : L'utilisateur " << utilisateur->getNom() << " doit renouveler son abonnement premium." << endl << endl;
	}

	//Si l'utilisateur est régulier, vérifier s'il est déjà dans un groupe
	else if (!(static_cast<UtilisateurRegulier*>(utilisateur)->estGroupe())) {
		utilisateurs_.push_back(utilisateur);
		static_cast<UtilisateurRegulier*>(utilisateur)->setEtatGroupe(true);
	}
	else
		cout << "Erreur : L'utilisateur " << utilisateur->getNom() << " n'est pas souscrit a un abonnement premium et est deja en groupe." << endl << endl;
	return *this;
}
...
{
	os << "L'evenement nomme : " << groupe.getNom() << " a coute un total (en terme de depenses groupee) de : " << groupe.getTotalDepenses() << ". Voici les utilisateurs et leurs depenses :" << endl << endl;
